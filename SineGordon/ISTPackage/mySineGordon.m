...
HillSG::usage="HillmKdV[q,n] gives the eigenvalues of the modified Zakharov-Shabat scattering (including continuous spectrum) problem with n Fourier modes using Hill's method.";
...
ScatteringMatrixFiniteSG[q_,n_,el_]:=Module[{qf,qfb,Dm,Dmb,IIm,IImb,P,Pb,id,Q,R,Qb,Rb,DM,DMb,q\[Sigma]1,q\[Sigma]1b,J\[Sigma]3,J\[Sigma]31,J\[Sigma]3b,J\[Sigma]31b,rhs,A,Ab},
       qf=Fun[q,Line[{-el,0}],n]; (*Forward scattering*)
qf=-0.5*qf';(*assume the diff is spectral*)
...
R= - DiagonalMatrix[qf//Values];
Qb = DiagonalMatrix[qfb//Values];
Rb = -DiagonalMatrix[qfb//Values];
...
ScatteringMatrixFiniteSG[qf,qfb,A,Ab,J\[Sigma]3,J\[Sigma]31,J\[Sigma]3b,J\[Sigma]31b,IIm,IImb]];


ScatteringMatrixFiniteSG[qf_,qfb_,A_,Ab_,J\[Sigma]3_,J\[Sigma]31_,J\[Sigma]3b_,J\[Sigma]31b_,IIm_,IImb_][w_]:=Module[{ans0,ans1,rhs,lhs,n,s1,s2,qb,q,r,rb},
q = qf//Values;
qb = qfb//Values;
r = -q;
rb= -qb;
...
HillSG[F_,n_]:=Module[{c2,c1,c0,f11,f12,zz,f02,f01,t,A,mu,L,P,dftx,fun},
Off[General::unfl];
Off[General::ovfl];
mu = .25;
L = Pi;
...
LocatePolesSG[F_,n_]:=Module[{eval,s,i,sys,vecs},
Off[General::unfl];
eval=HillSG[F,n];
s = {};
For[i=1,i<=Length[eval],i++,
If[Im[eval[[i]]]>10^(-4)&& Abs[eval[[i]]]<30,    (*what does this condition imply?*)
s=Join[s,{eval[[i]]}];
];
];
On[General::unfl];
...
\[Theta][x_,t_][z_]:=2 I (-t/4/z+x z);    (*\[Theta][x_,t_][z_]:=2 I (-t/4/z+x z);  *)
rb[k_]:= cc[r[cc[k]]];
...
G[x_,t_][z_]:=({
 {1+r[z]rb[z], rb[z]Exp[-\[Theta][x,t][z]]},
 {r[z] Exp[\[Theta][x,t][z]], 1}
});
G[_,_][_?InfinityQ]:=({
 {1., 0},
 {0, 1.}
...
Gsamp[x_,t_][z_]:=({
 {1+rsamp[z]rbsamp[z], rbsamp[z]Exp[-\[Theta][x,t][z]]},
 {rsamp[z]  Exp[\[Theta][x,t][z]], 1}
});
...
J[0][x_,t_]:={{G[x,t][#]&,M[x,t][#]&,M[x,t][#]&,P[x,t][#]&,P[x,t][#]&,G[x,t][#]&},{Line[{-el,-rad}],Line[{-rad,-rad*I}],Line[{-rad*I,rad}],Line[{rad,I*rad}],Line[{rad*I,-rad}],Line[{rad,el}]},{bigN,smallN,smallN,smallN,smallN,bigN}};
Jsamp[0][x_,t_]:={{G[x,t][#]&,M[x,t][#]&,M[x,t][#]&,P[x,t][#]&,P[x,t][#]&,G[x,t][#]&},{Line[{-el,-rad}],Line[{-rad,-rad*I}],Line[{-rad*I,rad}],Line[{rad,I*rad}],Line[{rad*I,-rad}],Line[{rad,el}]},{bigN,smallN,smallN,smallN,smallN,bigN}};
...
SGAuto[x_,t_]:=\[Piecewise]{
 {SG[0][x,t][[1]], Abs[x]<=5&&t<.1},
 {SG[0][x,t][[1]], t>tcrit    (*region 2*)},
 {SG[0][x,t][[1]], True              (*region 1*)}
}
